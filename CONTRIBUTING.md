# Contributing to mcp-invoicenow-sg
...
```bash
uv run ruff check src/mcp_invoicenow_sg/ tests/ audit/
uv run ruff format --check src/mcp_invoicenow_sg/ tests/ audit/
uv run mypy src
```
...
```bash
uv run ruff check --fix src/mcp_invoicenow_sg/ tests/ audit/
uv run ruff format src/mcp_invoicenow_sg/ tests/ audit/
```
...
- [ ] Validation fixes reference the relevant rule ID (e.g. `[NEED: PINT-SG rule id, from the supplied specification]`)
...
Please open an issue at https://github.com/cmendezs/mcp-invoicenow-sg/issues and include:
